...
def ingest_metric(metric_in: MetricIn, db: Session = Depends(get_db)):
    ...
    # Trigger SRE AI Agent Pipeline synchronously
    try:
        from agents.pipeline import run_full_pipeline
        run_full_pipeline(db_metric, db)
        
        # Check if the anomaly agent flags this as an anomaly
        ...
        print(f"Error running pipeline inside ingest_metric: {e}")
